...
import { LongAnswerContainer, mapDispatchToProps } from '../LongAnswerContainer';

test('it renders', t => {
  const wrapper = shallow(<LongAnswerContainer />);

  t.true(wrapper.exists('.long-answer-container'));
});
...
test('handleTitleChange works', t => {
  ...
  const wrapper = shallow(<LongAnswerContainer {...props} />);
